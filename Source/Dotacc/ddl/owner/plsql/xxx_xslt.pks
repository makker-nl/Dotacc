CREATE OR REPLACE PACKAGE xxx_xslt
AS
  /******************************************************************************
  Purpose         : Functionality to transform XML documents.
  ...
  *******************************************************************************/
  ...
  PRAGMA EXCEPTION_INIT(process_error, -20100);
  --
TYPE t_name_value
IS
  RECORD
  (
    ...
    VALUE VARCHAR2(200));
  --
  -- Parameter List as a table of name value pairs.
TYPE t_paramlist
IS
  TABLE OF t_name_value INDEX BY BINARY_INTEGER;
  --
  ...
  FUNCTION new_stylesheet_From_URL(
    ...
    RETURN xslprocessor.Stylesheet;
  --
  -- Methods to transform an XML Document with an XSLT stylesheet, returning a String
  --
