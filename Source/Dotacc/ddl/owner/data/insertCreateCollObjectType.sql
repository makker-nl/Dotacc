...
set define off
declare
  l_xsl_name xxx_xml_documents.docname%type := 'CreateCollObjectType_1.1.xsl';
...
(
  ...
  <xsl:template name="CreateConstructors" match="FkConstraint" mode="CreateConstructors">
    ...
    <xsl:value-of select="concat($newLine,'-- Constructor for FK Constraint ',$constraintName)"/>
  ...
  <xsl:template name="SetFKColDecl" match="FkConstraint" mode="SetFKColDecl">
    ...
    <xsl:value-of select="concat($newLine, '-- Method to set columns for FK Constraint ',$constraintName,' on all members of collection.')"/>
  ...
  <xsl:value-of select="concat($newLine, '-- Perform Insert of the collection data')"/>
    <xsl:value-of select="concat($newLineComma,$memberProcDecl, $insertMemberName)"/>
  </xsl:template>
  <!-- delete Method -->
  <xsl:template name="DeleteDecl">
  <xsl:value-of select="concat($newLine, '-- Perform Delete of the collection data')"/>
  ...
  <xsl:value-of select="concat($newLine, '-- Perform a Cascading Delete of the collection data')"/>
  ...
  <xsl:template name="AddTypeDecl">
    ...
    <xsl:value-of select="concat($newLine, '-- Add ',$parameterType,' to ', $tableAttribute)"/>
  ...
  <xsl:value-of select="concat($newLine, '-- Return the collection as XML')"/>
