...
set define off
declare
  l_xsl_name xxx_xml_documents.docname%type := 'CreateCollObjectTypeBody_1.1.xsl';
...
-->
  ...
  <xsl:template name="CreateConstructors" match="FkConstraint" mode="CreateConstructors">
    ...
    <xsl:value-of select="concat('--', $newLine, '-- Constructor for FK Constraint ',$constraintName,$newLine)"/>
  ...
  <xsl:template name="SelectDerivationAttributes" match="DerivedColumn" mode="SelectDerivationAttributes">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:value-of select="concat($newLine,'                      -- Ignore construction of derivation attribute ' , $derivationAttribute)"/>
        <xsl:value-of select="concat($newLine,'               ,      null ')"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
}';
  l_xsl_content_b clob  := q'{  <!-- Generate Select of LookUp FK Attributes -->
  <xsl:template name="SelectFKAttributes" match="FkConstraint" mode="SelectFKAttributes">
    ...
    <xsl:value-of select="concat($newLine,'                      -- Construct Lookup attribute based on foreinkey to ' , $tableName)"/>
  ...
  <xsl:template name="SelectRefFKAttributes" match="ReferingForeignKey" mode="SelectRefFKAttributes">
    ...
    <xsl:value-of select="concat($newLine,'                      -- Construct Collection attribute based on foreinkey to ' , $tableName)"/>
    <xsl:call-template name="toLowerCase">
  ...
  <xsl:template name="SetFKColDecl" match="FkConstraint" mode="SetFKColDecl">
    ...
    <xsl:value-of select="concat('--', $newLine, '-- Method to set columns for FK Constraint ',$constraintName,' on all members of collection.',$newLine)"/>
  ...
  <xsl:template name="InsertDecl">
    ...
    <xsl:value-of select="concat('--', $newLine, '-- Perform Insert of the collection data', $newLine)"/>
  ...
  <xsl:template name="DeleteDecl">
    ...
    <xsl:value-of select="concat('--', $newLine, '-- Perform Delete of the collection data', $newLine)"/>
  ...
  <xsl:template name="DeleteCascDecl">
    ...
    <xsl:value-of select="concat('--', $newLine, '-- Perform Cascading Delete of the collection data', $newLine)"/>
  ...
  <xsl:template name="AddTypeDecl">
    ...
    <xsl:value-of select="concat('--', $newLine, '-- Add ',$parameterType,' to ', $tableAttribute,$newLine)"/>
    <xsl:value-of select="concat($memberProc, $memberName,'(',$parameter,$space, $parameterType,')',$memberProcClosure)"/>
    <xsl:value-of select="$memberBeginBody"/>
    <xsl:value-of select="concat('    -- Initialize table when not constructed yet.',$newLine)"/>
  ...
  <xsl:template name="ToXMLDecl">
    ...
    <xsl:value-of select="concat('--', $newLine, '-- Return the collection as XML',$newLine)"/>
